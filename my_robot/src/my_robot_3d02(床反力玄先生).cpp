...
float mass_all;
float g;
MatrixXd iden(3,3),iden2(3,6),mass(3,1),length(3,1),length_g(3,1),inertia(2,1),phi(6,1),kp(3,3),kd(3,3),q(3,1),q_dot(3,1),q_2dot(3,1),u(3,1),qd(3,1),qd_dot(3,1),qd_2dot(3,1);
MatrixXd F(3,1),Hb(3,3),Hbj(3,3),Hj(3,3),Cb(3,1),Cj(3,1),Gb(3,1),Gj(3,1),Jj(3,3),Jj_dot(3,3),J(6,3),H(6,6),C(6,1),G(6,1);
...
void cal_dynamics(MatrixXd q,MatrixXd q_dot)
{
	...
	cout << "aa" << endl;
	Eigen::PartialPivLU<Eigen::MatrixXd> lu2(iden2-Jj.transpose()*J.transpose());
	cout << "bb" << endl;
	Eigen::PartialPivLU<Eigen::MatrixXd> lu3(J);

	//u = (Hj-Hbj.transpose()*lu1.solve(Hbj))*q_2dot+Hbj.transpose()*lu1.solve(iden*F-Cb-Gb)+Cj+Gj-Jj.transpose()*F;

	u = lu2.solve((Hj-Hbj.transpose()*lu1.solve(Hbj))*q_2dot+Hbj.transpose()*lu1.solve(iden*F-Cb-Gb)+Cj+Gj-Jj.transpose()*J.transpose()*(C+G-H*lu3.solve(Jj_dot*q_dot)));
